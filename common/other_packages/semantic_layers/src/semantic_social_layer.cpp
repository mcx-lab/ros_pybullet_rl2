...
{
void SemanticSocialLayer::onInitialize()
{
  ros::NodeHandle nh("~/" + name_), g_nh;
  current_ = true;
  first_time_ = true;
  people_sub_ = nh.subscribe("/people", 1, &SemanticSocialLayer::peopleCallback, this);
}

void SemanticSocialLayer::peopleCallback(const semantic_layers::People& people)
{
...
void SemanticSocialLayer::updateBounds(double origin_x, double origin_y, double origin_z, double* min_x, double* min_y,
                               double* max_x, double* max_y)
{
  ...
  {
    semantic_layers::Person& person = people_list_.people[i];
    semantic_layers::Person tpt = person;
    ...
    try
    {
      ...
      buffer_.transform(pt, opt, global_frame);
