...
#include <semantic_layers/semantic_proxemic_layer.h>
#include <math.h>
#include <angles/angles.h>
#include <pluginlib/class_list_macros.h>
#include <algorithm>
#include <list>
PLUGINLIB_EXPORT_CLASS(semantic_layers::SemanticProxemicLayer, costmap_2d::Layer)
...
{
void SemanticProxemicLayer::onInitialize()
{
  SemanticSocialLayer::onInitialize();
  ros::NodeHandle nh("~/" + name_), g_nh;
  server_ = new dynamic_reconfigure::Server<ProxemicLayerConfig>(nh);
  f_ = boost::bind(&SemanticProxemicLayer::configure, this, _1, _2);
  server_->setCallback(f_);
}

void SemanticProxemicLayer::updateBoundsFromPeople(double* min_x, double* min_y, double* max_x, double* max_y)
{
  std::list<semantic_layers::Person>::iterator p_it;
  costmap_2d::Costmap2D* costmap = layered_costmap_->getCostmap();

  for (p_it = transformed_people_.begin(); p_it != transformed_people_.end(); ++p_it)
  {
    semantic_layers::Person person = *p_it;
...
void SemanticProxemicLayer::updateCosts(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  ...
  for (p_it = transformed_people_.begin(); p_it != transformed_people_.end(); ++p_it)
  {
    ...
    double people_cost = person.cost; // costmap->getCost(mx, my);

    double angle = atan2(person.velocity.y, person.velocity.x);
    double mag = sqrt(pow(person.velocity.x, 2) + pow(person.velocity.y, 2));
    double factor = 1.0 + mag * factor_;
    double base = get_radius(cutoff_, people_cost, covar_);
    double point = get_radius(cutoff_, people_cost, covar_ * factor);
    ...
    for (int i = start_x; i < end_x; i++)
    {
      for (int j = start_y; j < end_y; j++)
      {
        ...
        double a;
        if (fabs(diff) < M_PI / 2)
          a = gaussian(x, y, cx, cy, people_cost, covar_ * factor, covar_, angle);
        else
          a = gaussian(x, y, cx, cy, people_cost, covar_,       covar_, 0);
...
void SemanticProxemicLayer::configure(ProxemicLayerConfig &config, uint32_t level)
{
