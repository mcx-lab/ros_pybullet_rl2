...
import numpy as np
import pybullet
from pybulletgym.envs.yc.robots import Omnirobot2
...
class OmniBase2BulletEnv(BaseBulletEnv):
  foot_ground_object_names = set(["floor"])  # to distinguish ground and other objects

  def __init__(self, robot, render):
    print("OmniBase2::__init__")
  ...
  def step(self, a):
    ...
    '''if state[1][0] != 0 and state[1][1] != 0 and state[1][2] != 0: # If no action taken, smaller punishment
    ...
    proximity_cost = 0
    if min(state[3]) <= self.detect_radius:
      proximity_cost -= self.kp_linear * (self.detect_radius - min(state[3]))

    '''if np.arctan2(abs(state[1][1]),state[1][0]) > 0.8: # redundant: or np.arctan2(abs(state[3]),state[2]) < 0: # contain the meaning of state[2] < 0.
    ...
    collision_cost = 0.0
    if max(state[2]) != 0:
      collision_cost -= self.kf_lower * max(state[2])
      if max(state[2]) >= 3.0:
...
class Omni2BulletEnv(OmniBase2BulletEnv):
  # as long as this class is called, it will have variables defined in OmniBaseBulletEnv as well.
  def __init__(self, render=False):
    self.robot = Omnirobot2()
    OmniBase2BulletEnv.__init__(self, self.robot, render)