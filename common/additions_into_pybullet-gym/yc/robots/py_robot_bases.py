import pybullet
import pybullet_data, importlib
...
class XmlBasedRobot:
	...
	def __init__(self, robot_name, action_dim, obs_dim, self_collision):
		...
		self.action_space = gym.spaces.Box(low=np.array([-1,-1,-2]), high=np.array([1,1,2]), dtype=np.float32) # [-2.0,-2.0,-pi] & [2.0,2.0,pi]
		# Not from -0.2655 to [0.2655,0.2655,0.2655] so as to give control over EBand velocity output. 
		
		# discrete action space
		# self.action_space = gym.spaces.MultiDiscrete([2,3,3]) # [2,3,3])
...
class URDFBasedRobot(XmlBasedRobot):
	...
	def reset(self, bullet_client):
		...
		self.episode += 1
		print("\nEpisode {}.\n".format(self.episode))
		self.robot_specific_reset(self._p, self.objects)
