...
from ros_pybullet_rl.msg import DepthImageProcessed, ForceTorqueSensors
import rospy
import random
...
class NavApfOmnirobot(OmniBase, URDFBasedRobot):
    random_yaw = False
    ...
    # def ros_shutdown_maneuvre(self):
    ...
    def apply_action(self, a):
        ...
        vel_cmd.linear.x = a[0] + self.nav_cmd_vel_info[0] + self.contact_delta_v.linear.x
        vel_cmd.linear.y = a[1] + self.nav_cmd_vel_info[1] + self.contact_delta_v.linear.y
    ...
    def calc_state(self):
        ...
        ### APF velocity
        # Exepcted input ~ 1-3kgf per sensor (in kgf)
        # formulae used: Sum kf*Fi
        contact_f_info = self.contact_info
        theta = 0.0
        angle_increment = np.pi / 3
        for force in contact_f_info:
            self.contact_delta_v.linear.x += -self.kf_lower * force * np.cos(theta)
            self.contact_delta_v.linear.y += -self.kf_lower * force * np.sin(theta)
