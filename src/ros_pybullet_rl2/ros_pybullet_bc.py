...
def run():
    ...
    if args.expert_data is not "":
        ...
        bc_trainer.train(n_epochs=100)# total_timesteps=args.n_timesteps)

        bc_trainer.save_policy(os.path.join(monitor_path, "final"))
    else:
        print("There is no expert data to run BC training, please check the expert_data parameter!")
