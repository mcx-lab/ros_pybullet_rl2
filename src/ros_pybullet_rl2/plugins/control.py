...
class pveControl:
    ...
    def __init__(self, joint_index, joint_name, controller_type):
        ...
        # rospy.loginfo("Type of name is: ", type(self._joint_name))
        # rospy.loginfo("Type of index is: ", type(self._joint_index))
        # rospy.loginfo("Joint name is: ", str(self._joint_name))
        # rospy.loginfo("Joint index is: ", str(self._joint_index))
    ...
    def pve_controlCB(self, msg):
        ...
        self.cmd = msg.data
        # if self.cmd:
        # 	rospy.loginfo("callback data received: ", self.cmd)
        ### it just cannot print output showing this error: TypeError: not all arguments converted during string formatting.
...
class Control:
    def __init__(self, pybullet, robot, **kargs):
        ...
        self.joint_index_name_dic = kargs['rev_joints'] # but they are in fact continuous joint?
        # setup subscribers
        self.pc_subscribers = []
        self.vc_subscribers = []
        self.ec_subscribers = []
        ...
        for joint_index in self.joint_index_name_dic:
            ...
            # rospy.loginfo("Type of name is: ", type(joint_name))
            # if joint_name:
            # 	rospy.loginfo("Type of name is: {}", format(type(joint_name)))
            # rospy.loginfo("Joint name is: ", str(joint_name))
            ...
            # create effort control object
            self.ec_subscribers.append(pveControl(joint_index, joint_name, 'effort'))
        rospy.loginfo("Successfully setup control plugin.")
    ...
    def execute(self):
        ...
        # rospy.loginfo("Checking if there is control data input...")
        ...
        for index, subscriber in enumerate(self.vc_subscribers):
            if subscriber.get_is_data_available():
                ...
                rospy.loginfo("Detected velocity control input.")
        ...
        if position_ctrl_task:
            ...
            rospy.loginfo("Running velocity control task.")
            self.pb.setJointMotorControlArray(bodyUniqueId=self.robot, jointIndices=self.joint_indices,
            ...
            rospy.loginfo("Running effort control task.")
            self.pb.setJointMotorControlArray(bodyUniqueId=self.robot, jointIndices=self.joint_indices,
