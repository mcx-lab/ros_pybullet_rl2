...
class RGBDCamera:
    def __init__(self, pybullet, robot, **kargs):
        ...
        self.pub_image_d = rospy.Publisher('d_image', Image, queue_size=1)
        self.pub_image_d_for_reading = rospy.Publisher('d_image_processed', DepthImageProcessed, queue_size=1)
        ...
        self.processed_image_msg = DepthImageProcessed()
        self.processed_image_msg.width = self.image_msg.width
        self.processed_image_msg.height = self.image_msg.height
        self.processed_image_msg.header.frame_id = self.image_msg.header.frame_id
        self.processed_image_msg.encoding = 'utf-8'
        self.processed_image_msg.is_bigendian = self.image_msg.is_bigendian
        self.processed_image_msg.step = self.image_msg.step
    ...
    def extract_frame_d(self, camera_image):
        far = 1000
        near = 0.01
        d_image = np.zeros((self.image_msg.height, self.image_msg.width, 1))

        camera_d_image = np.reshape(camera_image[3], (camera_image[1], camera_image[0], 1))

        d_image = far * near / (far - (far - near) * camera_d_image)

        far = 255 # to imitate 8 bytes of pixel data. 
        depth_image_list = np.float32(far * near / (far - (far - near) * np.array(camera_image[3])).round(decimals=3).flatten()).tolist()

        # return frame
        return d_image.astype(np.uint8), depth_image_list
    ...
    def execute(self):
        ...
        frame_d, depth_image_list = self.extract_frame_d(pybullet_cam_resp)
        # fill pixel data array
        self.image_msg.encoding = 'rgb8'
        self.image_msg.data = self.image_bridge.cv2_to_imgmsg(frame).data # convert an OpenCV cv::Mat type to a ROS sensor_msgs::Image message. 
        ...
        self.image_msg.data = self.image_bridge.cv2_to_imgmsg(frame_d).data # , encoding='mono8'
        self.image_msg.header.stamp = rospy.Time.now()
        self.pub_image_d.publish(self.image_msg)

        # print("The length of image data: ",len(depth_image_list))
        self.processed_image_msg.data = depth_image_list
        self.processed_image_msg.header.stamp = rospy.Time.now()
        self.pub_image_d_for_reading.publish(self.processed_image_msg)
