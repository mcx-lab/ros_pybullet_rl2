...
import ros_pybullet_rl2.utils.import_envs  # noqa: F401 pytype: disable=import-error
...
class NavOmnibaseController():
    """
    Generate expert data with controller
    """

    def __init__(self):
        self.omni_vel_info = np.zeros((3,), dtype=np.float32)

        self.omni_vel_sub = rospy.Subscriber('/joy', Joy, self.omni_vel_callback)

        self.nav_msg_received = 0
    

    def omni_vel_callback(self, data):
        # Set to publish at 10 Hz.
        max_turn = 2
        tmp = Twist()
        tmp.linear = Vector3(data.axes[1], data.axes[0], 0)
        tmp.angular = Vector3(0, 0, max_turn*data.axes[2])

        self.omni_vel_info[0] = tmp.linear.x
        self.omni_vel_info[1] = tmp.linear.y
        self.omni_vel_info[2] = tmp.angular.z

        self.nav_msg_received = 1
       

    def dummy_expert(self, _obs):
        while self.nav_msg_received == 0:
            print('Waiting for joystick data')
            pass

        action = np.zeros((3,), dtype=np.float32)
        action[0] = self.omni_vel_info[0]
        action[1] = self.omni_vel_info[1]
        action[2] = self.omni_vel_info[2] 
        a = np.array([action])
...
class RolloutAndSaveCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        """
        Call NavOmnibaseController
        """
        controller = NavOmnibaseController()

        if self.save_timesteps > 0 and self.n_calls % self.save_timesteps == 0:
            """
            Use controller.dummy_expert as the model to generate expert data
            """  
            trajs = rollout.generate_trajectories(controller.dummy_expert, self.model.env, self.sample_until)
            # if self.unwrap:
...
class ExperimentManager(object):
    ...
    def __init__(
        ...
        self.ros_pybullet_rl2_dir = rospy.get_param('~ros_pybullet_rl2_dir')
        ...
        self._is_atari = False # permanently disabled atari
        ...
        self.log_path = f"{self.ros_pybullet_rl2_dir}/{log_folder}/{self.algo}/"
    ...
    def setup_experiment(self) -> Optional[BaseAlgorithm]:
        ...
        is_render = rospy.get_param('~render')
        if is_render:
    ...
    def learn(self, model: BaseAlgorithm) -> None:
        ...
        try:
            ...
            rospy.logwarn("Training interrupted, killing training node now...")
            pass
        finally:
            # Release resources
            try:
    ...
    def read_hyperparameters(self) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        with open(ros_pybullet_rl2_dir + f"/hyperparams/{self.algo}.yml", "r") as f:
        ...
        if type(self.custom_hyperparams) is dict:
    ...
    # The model.learn here is for optuna trial
